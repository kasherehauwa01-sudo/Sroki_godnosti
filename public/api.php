...
function normalizeExpiryDate(string $value, string $rawValue = '', bool $forceInvalid = false, ?bool $forceFullDate = null): array
{
    ...
    if (isUnlimitedExpiryValue($raw) || isUnlimitedExpiryValue($value)) {
        // Поле batches.expiry_date исторически NOT NULL. Для бессрочной партии
        // сохраняем техническую дату, но ставим expiry_invalid = 1: все запросы
        // событий уже исключают такие строки и уведомления по ним не создаются.
        return [
            'date' => '9999-12-31',
            'full' => false,
            'invalid' => true,
...
function isUnlimitedExpiryValue(string $value): bool
{
    $normalized = mb_strtolower(trim($value), 'UTF-8');
    $normalized = preg_replace('/[\s\x{00A0}]+/u', ' ', $normalized) ?? $normalized;
    return in_array($normalized, ['не ограничен', 'не ограничено', 'не ограничена'], true);
}
