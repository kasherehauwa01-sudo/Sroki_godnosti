...
<script src="assets/batch-export-selection.js?v=20260825-01"></script>
<script>
...
async function downloadSelectedPurchaseEventBatches(selectedBatchIds) {
    let response;
    try {
        response = await fetch(`${purchaseEventApiUrl}?action=purchase_event_primary_invoice_xls`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ token: purchaseEventToken, selected_batch_ids: selectedBatchIds }),
        });
    } catch (_) {
...
function openPurchaseEventBatchSelection() {
    closePurchaseEventExportDialog();
    if (!purchaseEventData?.rows?.length) {
        document.querySelector('#purchaseEventError').textContent = 'Событие не найдено';
        return;
    }
