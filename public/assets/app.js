...
function expiryDateInfo(value) {
    ...
    if (/^не[\s\u00a0]+ограничен(?:о|а)?$/iu.test(raw)) {
        return { invalid: true, placeholder: '9999-12-31', raw: 'Не ограничен', full: false, unlimited: true };
    }
    ...
    return { invalid: false, placeholder: '', raw, full: false, unlimited: false };
}
...
function readXlsx(file) {
    ...
    reader.onload = (event) => {
        try {
            ...
            const exampleRows = state.importRows.slice(0, 3).map((row) => `${row.article} — ${row.code || 'без кода'} — ${row.name || 'без наименования'} — ${row.expiryInvalid ? (row.expiryRaw === 'Не ограничен' ? row.expiryRaw : `${row.expiryRaw} (некорректная дата)`) : formatExpiryMonthRu(row.expiryDate, row.expiryFullDate)}`).join('\n');
