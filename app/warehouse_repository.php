...
function groupEventBatchesByManager(PDO $pdo, array $batches): array
{
    $codes = array_values(array_unique(array_filter(array_map(
        static fn (array $batch): string => trim((string)($batch['code'] ?? '')),
        $batches
    ))));
    if (!$codes) {
        return [];
...
function groupBatchesByManagerAssignments(array $batches, array $assignments): array
{
    ...
    foreach ($batches as $batch) {
        $batchesByCode[mb_strtolower(trim((string)$batch['code']))][] = $batch;
    }

    $groups = [];
    foreach ($assignments as $assignment) {
        $email = mb_strtolower(trim((string)$assignment['manager_email']));
        $codeKey = mb_strtolower(trim((string)$assignment['code']));
        if ($email === '' || empty($batchesByCode[$codeKey])) {
            continue;
        }
        if (!isset($groups[$email])) {
            $groups[$email] = [
                'manager_name' => (string)$assignment['manager_name'],
                'manager_email' => $email,
                'batches' => [],
            ];
        }
        foreach ($batchesByCode[$codeKey] as $batch) {
            $groups[$email]['batches'][(int)$batch['id']] = $batch;
        }
    }
...
function loadCatalogVrManagerAssignments(PDO $pdo, array $codes): array
{
    ...
    $emailSelect = $catalog['email'] !== null
        ...
        : "''";
    $sql = sprintf(
        'SELECT %s AS product_code, %s AS manager_name, %s AS manager_email FROM %s WHERE TRIM(%s) IN (%s)',
    ...
    foreach ($rows as $row) {
        ...
        $assignments[] = [
            ...
            'manager_name' => $managerName !== '' ? $managerName : $managerEmail,
